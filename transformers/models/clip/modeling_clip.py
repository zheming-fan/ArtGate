...
class CLIPEncoderLayer(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,  # 输入的隐藏状态，形状为 (batch, seq_len, embed_dim)
        attention_mask: torch.Tensor,  
        causal_attention_mask: torch.Tensor, 
        output_attentions: Optional[bool] = False,  
        artifact_token: Optional[torch.FloatTensor] = None,
    ) -> Tuple[torch.FloatTensor]:


        residual = hidden_states  
        # print(hidden_states.shape)
        if artifact_token is not None and not torch.all(artifact_token == 0):
        # 检查 artifact_token 和 hidden_states 的形状是否兼容
            if hidden_states.shape[-1] == artifact_token.shape[-1]:  # 特征维度相同
                seq_len = hidden_states.shape[-2]
                artifact_token_len = artifact_token.shape[-2]
        
                if seq_len == 257:  # 初始拼接情况
                    # 在序列长度维度上拼接
                    hidden_states = torch.cat([hidden_states, artifact_token], dim=-2)
                    # 创建与artifact_token相同形状的零张量并拼接到residual
                    residual = torch.cat([residual, torch.zeros_like(artifact_token)], dim=-2)
                elif seq_len > 257:  # 后续加法融合情况
                    # 计算可以融合的长度（取两者最小长度）
                    fusion_len = min(seq_len - 257, artifact_token_len)
                    if fusion_len > 0:
                    # 不变
                        hidden_states[..., -fusion_len:, :] = hidden_states[..., -fusion_len:, :]
                    # # 自适应地在末尾进行加法融合
                    #     hidden_states[..., -fusion_len:, :] += artifact_token[..., -fusion_len:, :]
                    else:
                        raise ValueError(f"Invalid fusion length: {fusion_len}")
                else:
                    raise ValueError(f"Unexpected sequence length: {seq_len}")
            else:
                raise ValueError(f"Feature dimension mismatch: hidden_states {hidden_states.shape}, artifact_token {artifact_token.shape}")
...
class CLIPTEXTEncoder(nn.Module):
    ...
    def __init__(self, config: CLIPConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([CLIPTEXTEncoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        myfe: Optional[torch.FloatTensor] = None,
    ) -> Union[Tuple, BaseModelOutput]:
        r"""
        Args:
            inputs_embeds (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`):
...
class CLIPTextTransformer(nn.Module):
    def __init__(self, config: CLIPTextConfig):
        ...
        self.encoder = CLIPTEXTEncoder(config)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        myimg: Optional[torch.FloatTensor] = None,
        ...
        if myimg is not None:
            first_dimension = myimg.shape[0]
            if hidden_states.size(0) == 1:
                hidden_states = hidden_states.expand(first_dimension, *hidden_states.shape[1:])
            myimg = myimg.unsqueeze(1)
            hidden_states = hidden_states.clone()  # 深拷贝 hidden_states
            # print("Hidden states :", hidden_states)
            hidden_states[:, 5:6, :] = myimg
