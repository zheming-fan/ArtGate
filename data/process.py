...
class RandomMask(object):
    ...
    def __call__(self, tensor):
        ...
        if rd.random() > self.p: return tensor
        ...
        if self.fixed_ratio:
            ...
            ratio = rd.uniform(self.ratio[0], self.ratio[1])
        ...
        while len(selected_positions) < num_masks:
            top = rd.randint(0, (h // self.patch_size) - 1) * self.patch_size
            left = rd.randint(0, (w // self.patch_size) - 1) * self.patch_size
...
def processing_pfar(img, opt, name):
    if opt.isTrain:
...
def processing_safe(img, opt, name):
    if opt.isTrain:
        crop_func = transforms.RandomCrop(opt.CropSize)
    elif opt.no_crop:
        crop_func = transforms.Lambda(lambda img: img)
